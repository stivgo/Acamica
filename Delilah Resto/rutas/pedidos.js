        ...
        if (id_productos.length === cantidades.length) {
            for (let i = 0; i < id_productos.length; i++) {
                ...
                if (buscar) {
                    ...
                    res.status(404).json({ error: 'No existe el producto con el id ' + id_productos[i] })
                    return
                }
            }
            ...
            `
            try {
                data = await db.ejecutarConsulta(query, { id_usuario, fecha, estado, formaPago, total, direccion }, false)
            ...
            for (let i = 0; i < id_productos.length; i++) {
                ...
                `
                try {
                    await db.ejecutarConsulta(query2, { data, id_producto, cantidad }, false)
            ...
            `
            try {
                await db.ejecutarConsulta(queryActualizar, {totalCalculado,data}, false)
            } catch (error) {
                next(error)
            }
...
router.get('/pedidos', usuario.midVerificarToken, async(req, res, next) => {
    ...
    for (let i=0;i<data.length;i++){
        ...
        const queryDescripcion =`
                ...
                `
        try{
            let descripcion = await db.ejecutarConsulta(queryDescripcion,{id},true)
...
router.put('/pedidos/:id',usuario.midVerificarToken, usuario.verificarAdmin, async(req, res, next) => {
    ...
    if (!pedido) {
        res.status(404).json({ error: 'No existe el pedido con el id ingresado' })
        return
    }
    console.log(pedido)
    const query = `
