...
router.post('/registrarse', async(req, res, next) => {
    ...
    if (username && nombre && apellido && correo && telefono && direccion && contrasena) {
        if (emailRegex.test(correo)) {
            ...
            if (usernameExiste) {
                res.status(409).json({ mensaje: 'Existe un usuario con el mismo username' })
            } else if (correoExiste) {
                res.status(409).json({ mensaje: 'Existe un usuario con el mismo correo' })
            } else {
                const query = `
                ...
                `
                try {
                    const data = await db.ejecutarConsulta(query, { username, nombre, apellido, correo, telefono, direccion, contrasena }, false)
